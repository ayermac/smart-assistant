...
- [ ] CLI 可以启动并持续进行对话。
- [ ] agent loop 基于 `pi-agent-core` 运行，并通过 `pi-ai` 调用模型 provider。
- [ ] 助手可以流式输出回答。
- [ ] 工具调用链路可以运行至少一个本地工具。
- [ ] 会话可以保存并在后续恢复。
- [ ] 用户可以明确写入长期记忆。
- [ ] 助手可以根据查询回忆长期记忆。
- [ ] 助手可以 ingest 本地 Markdown/text 文件并建立第一版索引。
- [ ] 助手可以从本地知识库检索相关片段并用于回答。
- [ ] 检索不到结果时，助手会明确说明缺口，不编造本地知识。
- [ ] 助手可以把复杂任务拆成结构化步骤。
- [ ] 助手可以更新计划步骤状态。
- [ ] 10 个验收用例中至少 8 个可以稳定通过。
...
|----------|-----------|---------|
| CLI-first | 先验证 agent loop、memory、RAG、planning 的核心体验，避免 UI 提前放大范围 | — Pending |
| Local-first | 用户希望知识、记忆和会话默认可控、可审计 | — Pending |
| TypeScript + `pi-ai` + `pi-agent-core` | 与参考项目 `pi` 对齐，降低集成不确定性 | — Pending |
| Markdown/text RAG only in v1 | 本地文本知识库能覆盖第一版目标，避免文档解析范围失控 | — Pending |
| Memory and RAG separated | 长期事实和知识文档的生命周期、来源、更新方式不同 | — Pending |
| Single-agent MVP | 先把工具选择和状态管理跑稳，再考虑多 agent | — Pending |
